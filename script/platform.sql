create table lesson
(
    id         int auto_increment comment '课程标识，自增id'
        primary key,
...
(
    id         int auto_increment comment '学生标识，自增id'
        primary key,
...
create table study
(
    student_id int       default -1                not null comment '学生标识，自增id',
    lesson_id  int       default -1                not null comment '课程标识，自增id',
    created_at timestamp default CURRENT_TIMESTAMP not null comment '创建时间',
    updated_at timestamp default CURRENT_TIMESTAMP not null on update CURRENT_TIMESTAMP comment '更新时间'
...
(
    id         int auto_increment comment '题目标识，自增id'
        primary key,
    ...
    is_enable  int(2) default 0 not null comment '是否启用 1.启用 0.停用',
    created_at timestamp    default CURRENT_TIMESTAMP not null comment '创建时间',
    updated_at timestamp    default CURRENT_TIMESTAMP not null on update CURRENT_TIMESTAMP comment '更新时间'
...
(
    id         int auto_increment comment '老师标识，自增id' primary key,
